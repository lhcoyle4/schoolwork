...
\brief
This file contains the header file for the Memory Manager C++ assignment.
*/
/******************************************************************************/
//---------------------------------------------------------------------------
...
class ObjectAllocator
{
  ...
  private:
    ...
    void ExpandPages(void) NOEXCEPT(false);
    bool CheckIfFree(void* Object) const;
    byte_t* increment_head(byte_t* head) const;
    void push_front(byte_t* &head, byte_t* newhead);
    void setup_header(void);
    void update_header(bool inuse, byte_t* Object, const char* label = 0);
    bool validate_object(byte_t const* Object) const;
    void delete_memblock(void* Object);
    unsigned node_size; 
    unsigned offset;    
    OAConfig oaconfig;  // the config parameters
    OAStats oastats;    // the stats parameters
    byte_t* FreeList;   // the beginning of the list of objects
    byte_t* PageList;   // the beginning of the list of pages
};
